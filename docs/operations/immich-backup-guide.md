...
.\scripts\windows\backup\backup-immich.ps1
...
.\scripts\windows\backup\backup-immich.ps1 -StopServer $true
```
...
chmod +x scripts/linux/restore/restore-immich.sh

# Run restore
sudo ./scripts/linux/restore/restore-immich.sh /path/to/immich_backup_2026-01-23_174752
```
...
powershell.exe -ExecutionPolicy Bypass -File "C:\Users\Blake\Docker\MediaServer\scripts\windows\backup\backup-immich.ps1" -StopServer $true
```

```bash
# Linux cron - Weekly backup (after migration)
0 2 * * 0 /opt/immich/scripts/linux/backup/backup-volumes.sh > /var/log/immich-backup.log 2>&1
```
